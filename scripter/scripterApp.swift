...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if let button = statusItem.button {
            ...
            button.image = NSImage(systemSymbolName: "suit.spade.fill", accessibilityDescription: "Script Runner")
    ...
    func setupMenu() {
        ...
        let runScriptItem = NSMenuItem(title: "Run Script", action: nil, keyEquivalent: "")
        ...
        menu.addItem(NSMenuItem(title: "Create New Script", action: #selector(createNewScript), keyEquivalent: "n"))
        
        menu.addItem(NSMenuItem(title: "Manage Scripts", action: #selector(manageScripts), keyEquivalent: "m"))
    ...
    func updateRunScriptMenu(_ menu: NSMenu) {
        ...
        if scripts.isEmpty {
            let noScriptsItem = NSMenuItem(title: "No scripts available", action: nil, keyEquivalent: "")
            noScriptsItem.isEnabled = false
            menu.addItem(noScriptsItem)
        } else {
            for script in scripts {
                let item = NSMenuItem(title: script.name, action: #selector(runScript(_:)), keyEquivalent: "")
    ...
    @objc func createNewScript() {
        ...
        let inputTextField = NSTextField(frame: NSRect(x: 0, y: 40, width: 300, height: 24))
        inputTextField.placeholderString = "Script name"
        
        let scriptTextView = NSTextView(frame: NSRect(x: 0, y: 0, width: 300, height: 120))
        scriptTextView.string = "#!/bin/bash\n\n# Your script here\necho \"Hello, World!\""
        ...
        let stackView = NSStackView(frame: NSRect(x: 0, y: 0, width: 300, height: 164))
...
class ScriptManager: ObservableObject {
    ...
    init() {
        ...
        scriptsDirectory = appSupport.appendingPathComponent("MenuBarScriptRunner/Scripts")
    ...
    func loadScripts() {
        do {
            let fileURLs = try FileManager.default.contentsOfDirectory(at: scriptsDirectory, includingPropertiesForKeys: nil)
    ...
    func runScript(_ script: Script) async {
        ...
        do {
            ...
            await MainActor.run {
                ...
                resultAlert.informativeText = "Exit Code: \(process.terminationStatus)"
